...
class LoaderNote(object):
    def __init__(self, self_feature, partner_feature, args):
        self.dataset_self = np.array(self_feature, dtype=object)
    ...
    def batch_step(self, prev_pred=None):
        ...
        if len(self.rl_index) == 0:
            self.current_samples = [[self.train_dataset_self[i].astype(np.int64),
                                     self.train_dataset_partner[i].astype(np.int64)]
        ...
        result = []
        for var in [self_left, self_length, partner_left, partner_length, recover_idx, partner_central,
                         meta_central, output, partner_right, pr_length, pr_recover_idx]:
            if torch.is_tensor(var[0]):
                var = torch.Tensor(var).long().cuda()
                result.append(var)
            else:
                result.append(torch.from_numpy(np.array(var).astype(np.int64)).cuda())
    ...
    def get_rewarder_samples(self):
        ...
        def prepare_batch(start_ind, positive):
            ...
            if positive:
                ...
                logp = torch.Tensor(logp).long().cuda()
